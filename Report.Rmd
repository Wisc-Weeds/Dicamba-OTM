---
title: "Modeling Dicamba Injury in non-Xtend soybean"
author: "| Max Oliveira^1^, Guilherme Alves^2^, Rodrigo Werle^1^, Greg Kruger^2^ and Ryan Rector^3^ \n| ^1^ University
  of Wisconsin-Madison\n| ^2^ University of Nebraska-Lincoln  ^3^ Bayer Crop Science\n"
date: ''
output:
  pdf_document:
    fig_caption: yes
  ...
  html_document:
    df_print: paged
subtitle: Data analysis from Arkansas, Indiana, Michigan, Nebraska, Ontario and Wisconsin
---
...
region <-c("Wisconsin", "Nebraska", "Michigan", "Arkansas", "Indiana", "Ontario")
Latitude <-c(43.32861,41.474664768,  42.65889, 35.0834, 39.788663512, 42.437664916)
Longitude <-c(-89.31556, -100.507664636, -84.07222, -90.255, -87.368831858,  -81.883829798)
...
df <- subset(states, region %in% c("minnesota","wisconsin", "nebraska", "arkansas",  "michigan", "indiana", "ohio", "illinois", "missouri", "iowa"))
```

```{r include=FALSE}
counties <- map_data("county")
midwest <- subset(counties, region %in% c("minnesota","wisconsin", "nebraska", "arkansas",  "michigan", "indiana", "ohio", "illinois", "missouri", "iowa"))
```

```{r map, echo=FALSE, fig.height= 10, fig.width=12, fig.cap="Experiment locations of the large volatility dicamba study in 2018"}
ggplot(data = df, mapping = aes(x = long, y = lat, group = group, color=region)) + 
  geom_point(data=Map, mapping=aes(x=Longitude, y=Latitude, group=region, color=region), size=4) +
  geom_polygon(data = midwest, fill = "NA", color = "gray") +
  geom_polygon(color = "black", fill = NA) + scale_color_manual(values=c("red", "black", "purple", "green", "blue", "orange")) +
  labs(fill="", y="", x="", caption="") +
  #north(df, location = 'topright', symbol = 1, anchor = c(x = -103, y = 50)) +
  theme_map() +
  annotate("text", x = -100, y = 33, label = "Figure: University of Wisconsin-Madison Weed Sciece", size=4) +
      theme(axis.text=element_blank(), 
        axis.title=element_blank(), 
        panel.background = element_rect(fill="white", color = "black"), 
        panel.grid.major = element_line(color = "white"), 
  panel.grid.minor = element_blank(),
  legend.position=c(0.8,0.1), legend.text = element_text(size = 16, colour = "black"), legend.key = element_rect(fill=NA), panel.border = element_rect(fill=NA,color="black", size=0.5, linetype="solid"),
...
The study was conducted in a soybean field at Proctor, Arkansas (Figure 2)
...
Data from deposition of dicamba particles on paper filters was fitted to exponential decay model.

\newpage

## Wind

Frequency of wind direction and speed (m/s) three days after dicamba application (Figure 3).
...
Table: Estimation of the distance (m) that resulted in 1 (ED1), 10 (ED10), and 20% (ED20) injury on non-Xtend soybean at westupwind transects  (1, 2, and 3) at 22 DAT in Arkansas.
...
Table: Estimation of the distance (m) that resulted in 1 (ED1), 10 (ED10), and 20% (ED20) injury on non-Xtend soybean at east downwind transects (1, 2, and 3) at 22 DAT in Arkansas.

| Transect | ED1 (m) (± SE) | ED10 (m) (± SE)| ED20 (m) (± SE)
| ------------- | ------------- | ---------- |---------- |
| East Transect 1 | 215.55 ± 22.21 | 104.09 ± 4.36 | 78.32 ± 2.95
| East Transect 2 | 350.70 ± 41.41 | 127.46 ± 6.04 | 86.23 ± 4.23
| East Transect 3 | 358.72 ± 47.04 | 105.18 ± 7.22 | 65.44 ± 6.30
...
Table: Estimation of the distance (m) that resulted in 1 (ED1), 10 (ED10), and 20% (ED20) injury on non-Xtend soybean of four sides of the treated area (N, E, S, W) at 29 DAT in Arkansas.
...
## Transects (paper filter)

```{r, echo=FALSE, fig.cap="Deposition of dicamba particles at the transects in Arkansas", out.width = '100%', fig.align="center", fig.retina = 2, fig.pos='h'}
knitr::include_graphics("ARtransect.png")
```
...
The study was conducted in a soybean field at Montezuma, Indiana (Figure 10).
...
Frequency of wind direction and speed (m/s) three days after dicaamba application (Figure 11).
...
| ED50  | Uncover | 2.71 ± 0.17 | 15.71 | 0.00 ***
  ...
  Table: Estimation of the distance (m) that resulted in 1 (ED1), 10 (ED10), and 20% (ED20) injury on non-Xtend soybean of north direction (cover and uncover) at 21 DAT in Indiana.
...
| ED50  | Uncover | 2.71 ± 0.17 | 15.71 | 0.00 ***
  ...
  Table: Estimation of the distance (m) that resulted in 1 (ED1), 10 (ED10), and 20% (ED20) injury on non-Xtend soybean of middle (cover and uncover) at 21 DAT in Indiana.
...
```{r echo=FALSE, fig.height= 6, fig.width=6, fig.cap="Non-Xtend soybean injury (%) with distance (m) from the dicamba treated area of middle (cover and uncover) at 21 DAT in Indiana."}
...
| ED50  | Uncover | 2.42 ± 0.10 | 23.6 | 0.00 ***
  ...
  Table: Estimation of the distance (m) that resulted in 1 (ED1), 10 (ED10), and 20% (ED20) injury on non-Xtend soybean of south (cover and uncover) at 21 DAT in Indiana.
...
```{r IN21S2Plot,warning=FALSE, echo=FALSE, fig.height= 6, fig.width=6, fig.cap="Non-Xtend soybean injury (%) with distance (m) from the dicamba treated area of south (cover and uncover) at 21 DAT in Indiana."}
...
Table: Estimation of the distance (m) that resulted in 1 (ED1), 10 (ED10), and 20% (ED20) injury on non-Xtend soybean of transect north, middle, and south at 21 DAT in Indiana.
...
```{r warning=FALSE, echo=FALSE, fig.height= 6, fig.width=6, fig.cap="Non-Xtend soybean injury (%) with distance (m) from the dicamba treated area of transect (north, middle, and south) at 21 DAT in Indiana."}
...
```{r echo=FALSE, fig.height= 6, fig.width=6, warning=FALSE, fig.cap="Non-Xtend soybean height (cm) with distance (m) from the dicamba treated area of north (cover and uncover) at 21 DAT in Indiana."}
...
```{r echo=FALSE, fig.height= 6, fig.width=6, warning=FALSE, fig.cap="Non-Xtend soybean height (cm) with distance (m) from the dicamba treated area of middle (cover and uncover) at 21 DAT in Indiana."}
INH=filter(Height, state=="Indiana" & wind=="downwind")
...
```{r echo=FALSE, fig.height= 6, fig.width=6, warning=FALSE, fig.cap="Non-Xtend soybean height (cm) with distance (m) from the dicamba treated area of transect north, middle, and south at 21 DAT in Indiana."}
INH=filter(Height, state=="Indiana" & wind=="downwind")
...
The study was conducted in 53 of a 300 acre field at Van Gilder Farms Fowlerville, Michigan (Figure 21).
...
Frequency of wind direction and speed (m/s) three days after dicamba application (Figure 22).
...
#### Area located at downwind Area 1 (cover and uncover)
...
Table: Parameters of the log-logistic model at Area 1 (cover and uncover) at 21 DAT in Michigan.
...
Table: Estimation of the distance (m) that resulted in 1 (ED1), 10 (ED10), and 20% (ED20) injury on non-Xtend soybean at Area 1 (cover and uncover) at 21 DAT in Michigan.
...
Table: Comparison of EDs between UNcover and non cover areas in non-Xtend soybean at Area 1 (cover and uncover) at 21 DAT in Michigan.
...
#### Area located at downwind Area 2 (cover and uncover)
...
Table: Parameters of the log-logistic model at Area 2 (cover and uncover) at 21 DAT in Michigan.
...
Table: Estimation of the distance (m) that resulted in 1 (ED1), 10 (ED10), and 20% (ED20) injury on non-Xtend soybean at Area 2 (cover and uncover) at 21 DAT in Michigan.
...
Table: Comparison of EDs between uncover and non cover areas in non-Xtend soybean at Area 2 (cover and uncover) at 21 DAT in Michigan.
...
```{r echo=FALSE, fig.height= 6, fig.width=6, fig.cap="Non-Xtend soybean injury (%) with distance (m) from the dicamba treated area at Area 2 (cover and uncover) at 21 DAT in Michigan."}
...
#### Area located at downwind Area 3 (cover and uncover)
...
Table: Parameters of the log-logistic model at Area 3 (cover and uncover) at 21 DAT in Michigan.
...
Table: Estimation of the distance (m) that resulted in 1 (ED1), 10 (ED10), and 20% (ED20) injury on non-Xtend soybean at Area 3 (cover and uncover) at 21 DAT in Michigan.
...
Table: Comparison of EDs between UNcover and non cover areas in non-Xtend soybean at Area 3 (cover and uncover) at 21 DAT in Michigan.
...
```{r echo=FALSE, fig.height= 6, fig.width=6, fig.cap="Non-Xtend soybean injury (%) with distance (m) from the dicamba treated area at Area 3 (cover and uncover) at 21 DAT in Michigan."}
...
#### Area located in the **transect** (1, 2, and 3)


```{r include=FALSE, echo=FALSE, warning=FALSE}
MI4=filter(MI, Tarp=="No")
MI4$Location<- factor(MI4$Location, levels=c("A", "B", "C"), labels=c("1", "2", "3"))
...
Table: Parameters of the log-logistic model at transect (1, 2, and 3) locations at 21 DAT in Michigan.

|Parameter | Transect | Estimate (± SE) | t-value | p-value |
| ------------- | ------------- | ---------- |---------- |---------- |   
| Slope  | 1  |  6.93  ±  1.65 | 4.19 | 0.00 ***
| Slope  | 2  |  0.69  ±  0.19 | 3.49 | 0.00 **    
| Slope  | 3  |  1.27  ±  0.18 | 6.98 | 0.00 ***
| Asymptote  | 1 | 50.84  ±  1.14 | 44.26 | 0.00 ***
| Asymptote  | 2 |  17.75  ±  2.63 | 6.74 | 0.00 ***
| Asymptote  | 3 |  38.25  ±  2.72 | 14.02 | 0.00 ***
| ED50  | 1 |  10.73 ±   0.82 | 12.93 | 0.00 ***
| ED50  | 2  |  3.43 ±   1.93 | 1.77 | 0.08 .  
| ED50  | 3  |  5.70 ±   1.14 | 4.96 | 0.00 ***
  
  
  
Table: Estimation of the distance (m) that resulted in 1 (ED1), 10 (ED10), and 20% (ED20) injury on non-Xtend soybean at transect (1, 2, and 3) locations at 21 DAT in Michigan.

| Transect | ED1 (m) (± SE) | ED10 (m) (± SE)| ED20 (m) (± SE)
| ------------- | ------------- | ---------- |---------- |
|1 | 18.87  ±  3.35 | 13.15  ±  1.38 | 11.42 ±   0.95
|2 | 196.98  ±  217.98 | 2.38  ±  1.44 | NA
|3 | 98.20  ±  33.82 | 12.90  ±  2.19 | 5.30  ±  1.09




Table: Comparison of EDs among transect (1, 2, and 3) locations at 21 DAT in Michigan.

|Comparison | Estimate (± SE) | t-value | p-value |         
| ------------- | ------------- | ---------- |---------- |
|1/2:E1/E1 | 0.09 ± 0.10 | -8.42  | 0.00
|1/3:E1/E1 | 0.19 ± 0.07 | -1.08 |  0.00
|2/3:E1/E1 | 2.00 ± 2.32 | 0.43 | 0.66
|1/2:E10/E10 | 5.51 ± 3.38 |1.33  | 0.19
|1/3:E10/E10 | 1.01 ± 0.20 | 0.09 | 0.92
|2/3:E10/E10 | 0.18 ± 0.11 |-7.01 | 0.00
|1/2:E20/E20 | NA  | NA |  NA 
|1/3:E20/E20 | 2.15 ± 0.48 |0.02 | 0.00
|2/3:E20/E20 | NA  | NA  | NA 
...
newdata1 <- data.frame(Location ="1", newdata)
newdata2 <- data.frame(Location ="2", newdata)
newdata3 <- data.frame(Location ="3", newdata)
...
```{r echo=FALSE, fig.cap="Non-Xtend soybean injury (%) with distance (m) from the dicamba treated area in the at transect (1, 2, and 3) locations at 21 DAT in Michigan.", fig.height=6, fig.width=6}
...
```{r echo=FALSE, warning=FALSE, fig.height= 6, fig.width=6, fig.cap="Non-Xtend soybean height (cm) with distance (m) from the dicamba treated area at Area 1 (cover and uncover) at 21 DAT in Michigan."}
...
```{r echo=FALSE, warning=FALSE, fig.height= 6, fig.width=6, fig.cap="Non-Xtend soybean height (cm) with distance (m) from the dicamba treated area at Area 2 (cover and uncover) at 21 DAT in Michigan."}
MIH=filter(Height, state=="Michigan")
...
```{r echo=FALSE, warning=FALSE, fig.height= 6, fig.width=6, fig.cap="Non-Xtend soybean height (cm) with distance (m) from the dicamba treated area at Area 3 (cover and uncover) at 21 DAT in Michigan."}
MIH=filter(Height, state=="Michigan")
...
```{r echo=FALSE, fig.cap="Non-Xtend soybean height (cm) with distance (m) from the dicamba treated area in the at transect (1, 2, and 3) locations at 21 DAT in Michigan.", fig.height=6, fig.width=6, warning=FALSE}

MIH=filter(Height, state=="Michigan")

MIH4 <- MIH %>% 
  filter(location == "Transect A" | location == "Transect B" | location == "Transect C")
MIH4$location <- factor(MIH4$location, levels=c("Transect A", "Transect B", "Transect C"), labels=c("1", "2", "3"))
...
The study was conducted in a soybean field at West Central Research and Extension of the University of Nebraska-Lincoln (Figure 32).
...
Frequency of wind direction and speed (m/s) three days after dicamba application (Figure 33).
...
```{r warning=FALSE, echo=FALSE, include=FALSE, fig.height= 6, fig.width=6}
...
The study was conducted in a soybean field at Dresden, Ontario in Canada (Figure 41).
...
Frequency of wind direction and speed (m/s) three days after dicamba application (Figure 42).
...
| ED50  | 3 | 10.5 ± 0.97 | 10.7 | 0.00 ***
  ...
  Table: Estimation of the distance (m) that resulted in 1 (ED1), 10 (ED10), and 20% (ED20) injury on non-Xtend soybean of uncover area (1, 2, 3) at 21 DAT in Ontario.
...
The study was conducted in a 8 acres soybean field at the University of Wisconsin-Madison (Figure 48).
...
Frequency of wind direction and speed (m/s) three days after dicamba application (Figure 49).
...
#### Area located in the downwind (Area 2) of the dicamba treated block
...
Table: Parameters of the log-logistic model at Area 2 (cover and uncover) at 28 DAT in Wisconsin.
...
Table: Estimation of the distance (m) that resulted in 1 (ED1), 10 (ED10), and 20% (ED20) injury on non-Xtend soybean at the Area 2 (cover and uncover) at 28 DAT in Wisconsin.
...
Table: Comparison of EDs of non-Xtend soybean at the Area 2 (cover and uncover) at 28 DAT in Wisconsin.
...
```{r warning=FALSE, echo=FALSE, fig.height= 6, fig.width=6, fig.width=6, fig.cap="Non-Xtend soybean injury (%) with distance (%) from the dicamba treated area 2 (cover and uncover) in Wisconsin at 28 DAT."}
...
#### Area located in the downwind (Area 3) of the dicamba treated block
...
Table: Parameters of the log-logistic model at the Area 3 (cover and uncover) at 28 DAT in Wisconsin.
...
Table: Estimation of the distance (m) that resulted in 1 (ED1), 10 (ED10), and 20% (ED20) injury on non-Xtend soybean at the Area 3 (cover and uncover) at 28 DAT in Wisconsin.
...
Table: Comparison of EDs of non-Xtend soybean at the Area 3 (cover and uncover) at 28 DAT in Wisconsin.
...
```{r echo=FALSE, fig.height= 6, fig.width=6, fig.cap="Non-Xtend soybean injury (%) with distance (%) from the dicamba treated area at Area 3 (cover and uncover) in Wisconsin at 28 DAT."}
...
#### Area located in the downwind (Area 4) of the dicamba treated block
...
Table: Parameters of the log-logistic model at the Area 4 (cover and uncover) at 28 DAT in Wisconsin.
...
| ED50  | Uncover  |  4.19  ±  0.11 | 37.9 | 0.00 ***
  ...
  Table: Estimation of the distance (m) that resulted in 1 (ED1), 10 (ED10), and 20% (ED20) injury on non-Xtend soybean at the Area 4 (cover and uncover) at 28 DAT in Wisconsin.
...
Table: Comparison of EDs of non-Xtend soybean at the Area 4 (cover and uncover) at 28 DAT in Wisconsin.
...
#### Area located in the upwind (Area 1) of the dicamba treated block
...
```{r warning=FALSE, echo=FALSE, fig.height= 6, fig.width=6, fig.width=6, fig.cap="Non-Xtend soybean injury (%) with distance (m) from the dicamba treated area at Area 1 (cover and uncover) in Wisconsin at 28 DAT."}
...
```{r echo=FALSE, warning=FALSE, fig.height= 6, fig.width=6, fig.cap="Non-Xtend soybean height (cm) with distance (%) from the dicamba treated area at Area 2 (cover and uncover) in Wisconsin at 28 DAT."}
WIH1 <- Wisconsin %>% 
...
```{r echo=FALSE, warning=FALSE, fig.height= 6, fig.width=6, fig.cap="Non-Xtend soybean height (cm) with distance (%) from the dicamba treated area at Area 3 (cover and uncover) in Wisconsin at 28 DAT."}
WIH2 <- Wisconsin %>% 
...
```{r echo=FALSE, warning=FALSE, fig.height= 6, fig.cap="Non-Xtend soybean height (cm) with distance (%) from the dicamba treated area at Area 4 (cover and uncover) in Wisconsin at 28 DAT."}
WIH3 <- Wisconsin %>% 
...
```{r echo=FALSE, warning=FALSE, fig.height= 6, fig.width=6, fig.cap="Non-Xtend soybean height (cm) with distance (%) from the dicamba treated area at Area 1 (cover and uncover) in Wisconsin at 28 DAT."}
WIH4 <- Wisconsin %>% 
...
Metodology based on EPA - [Click here for download](https://www.epa.gov/sites/production/files/2015-08/documents/ftt_add_guidance_reviewing_env_fate_studies.pdf)
...
```{r, echo=FALSE, fig.cap="Air samples analysis using the integrated horizontal flux method", out.width = '100%', fig.align="center", fig.retina = 2, fig.pos='h'}
knitr::include_graphics("Airsample.png")
```
...
```{r, echo=FALSE, fig.cap="Air samples analysis using the aerodynamic method", out.width = '100%', fig.align="center", fig.retina = 2, fig.pos='h'}
knitr::include_graphics("Airsample2.png")
```
